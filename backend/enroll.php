...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($stmt->fetchColumn() > 0) {
            echo json_encode(value: ['success' => false, 'message' => 'You are already enrolled in this course.']);
            exit();
        }

        // Insert into students table
        $stmt = $pdo->prepare(query: "INSERT INTO students (user_id, course_id) VALUES (:user_id, :course_id)");
        $stmt->execute(params: ['user_id' => $user_id, 'course_id' => $course_id]);
